	...
	handleLogIn: function () {
		...
		firebase.auth().signInWithEmailAndPassword(email, password).catch(function (error) {
			...
			if (error) {
				...
				that.setState({ errorMsg: error.message ? error.message : "Invalid email or password combination." });
			}
		});
		...
		this.unsubscribe = firebase.auth().onAuthStateChanged((user) => {
			...
			this.userRef.on("value", snap => {
				...
				if (snap.val().type == "student" && countDownDate < Date.now()) {
					console.log('fgjmyesssssssss@@@@@@@@@@@@')
					that.setState({ hasError: true });
					that.setState({ errorMsg: "Account Expired" });
				} else {
					if (user) {
						hashHistory.push("/");
					} else {
